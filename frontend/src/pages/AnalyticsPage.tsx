...
export function AnalyticsPage({ userId }: { userId: string }) {
  ...
  const { kpis, score_trend, gir_trend, putts_trend, scoring_by_par, scoring_by_handicap, gir_vs_non_gir, handicap_trend } = data;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
        ...
        {handicap_trend.some((r) => r.handicap_index != null) && (
          <ChartCard title="Handicap Index Trend">
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={handicap_trend} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
                <XAxis dataKey="round_index" tick={{ fontSize: 11, fill: "#9ca3af" }} tickLine={false} axisLine={false} />
                <YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} tickLine={false} axisLine={false} domain={["auto", "auto"]} reversed tickFormatter={(v: number) => v < 0 ? `+${Math.abs(v)}` : String(v)} />
                <Tooltip
                  contentStyle={{ fontSize: 12, borderRadius: 8, border: "1px solid #e5e7eb", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}
                  formatter={((v: number) => [v < 0 ? `+${Math.abs(v).toFixed(1)}` : v?.toFixed(1), "Handicap Index"]) as Fmt}
                />
